...
Version History:
	...
	2.2	05/07/2002	- Added automatic detection of Gzipped
				  tar files (Thanks go to J黵gen Falch
				  for the idea)
...
*/
	...
	*/
	...
	// Add a file to the tar archive
	function addPosFile(&$gdata) {

		if(empty($gdata['gid']))return false;

		$filename="$gdata[gid]#$gdata[rules]#$gdata[b_name]#$gdata[w_name]#$gdata[swaped]#$gdata[status].pos";
		if($this->containsFile($filename))
			return false;

		//生成pos文件
		$gdata['moves'] = substr( $gdata['moves'], 0, $gdata['mcount']);
		...
		$activeFile["size"]		= strlen($gdata['moves'])+1;
		$activeFile["time"]		= $gdata['l_time'];
		//$activeFile["checksum"]		= $checksum;
		$activeFile["user_name"]	= "";
		$activeFile["group_name"]	= "";
		$activeFile["file"]		= chr($gdata['mcount']);
		$activeFile["file"]	.=$gdata['moves'];

		return true;
	}

	function addInfo($buf)
	{
		if($this->containsFile('readme.txt'))
			return false;

		// Add file to processed data
		$this->numFiles++;
		$activeFile			= &$this->files[];
		$activeFile["name"]		= 'readme.txt';
		$activeFile["mode"]		= 0;
		$activeFile["user_id"]		=0;
		$activeFile["group_id"]		=0;
		$activeFile["size"]		= strlen($buf);
